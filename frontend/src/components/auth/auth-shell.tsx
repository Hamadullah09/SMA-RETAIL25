...
export function AuthShell({
  ...
  return (
    <div className="grid min-h-screen bg-surface lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
      ...
      <aside className="order-2 hidden flex-col justify-between border-r border-subtle bg-panel-sunken p-10 lg:order-1 lg:flex">
        <div>
          <p className="text-label font-medium uppercase tracking-widest text-ink-faint">Retail25</p>
          <p className="mt-6 max-w-sm text-h2 font-semibold leading-tight text-ink">
        ...
        <dl className="grid grid-cols-3 gap-6 border-t border-subtle pt-6">
          {[
            ...
            <div key={term}>
              <dt className="text-label uppercase tracking-wide text-ink-faint">{term}</dt>
              <dd className="mt-0.5 text-body text-ink-muted">{detail}</dd>
            </div>
          ))}
        </dl>
      </aside>
      ...
      <main className="order-1 flex items-center justify-center p-6 lg:order-2">
        <div className="w-full max-w-sm">
          ...
          <p className="mt-1 text-body text-ink-muted">{lead}</p>

          <div className="mt-8">{children}</div>

          {footer ? <div className="mt-6 text-body text-ink-muted">{footer}</div> : null}
        </div>
      </main>
    </div>
  );
