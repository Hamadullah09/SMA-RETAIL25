...
public sealed class StaffController : ControllerBase
{
    ...
    public async Task<IActionResult> Browse(
        [FromQuery] Guid locationId, [FromQuery] bool includeInactive = false, CancellationToken ct = default)
    ...
    public async Task<IActionResult> MyTimeClock([FromQuery] Guid locationId, CancellationToken ct)
    ...
    [HttpPut("time-clock/{id:guid}")]
    public async Task<IActionResult> AmendTimeClock(Guid id, [FromBody] AmendTimeClockRequest request, CancellationToken ct)
        => (await _sender.Send(new AmendTimeClockEntryCommand(id, request.ClockIn, request.ClockOut), ct))
            .ToActionResult(this);

    [HttpDelete("time-clock/{id:guid}")]
    public async Task<IActionResult> DeleteTimeClock(Guid id, CancellationToken ct)
    ...
    [HttpGet("{staffId:guid}/commission-rules")]
    public async Task<IActionResult> CommissionRules(Guid staffId, CancellationToken ct)
